...
import pytz # Librería para la hora correcta

# --- CONFIGURACIÓN ---
# Cambia 'America/Mexico_City' por la de tu país (ej: 'America/Bogota', 'America/Argentina/Buenos_Aires')
...
# --- 5. INVENTARIO ---
    ...
    if role == "admin":
        with t1:
            with st.form("f_reg", clear_on_submit=True):
                ...
                f_ing = st.date_input("Fecha de Ingreso", ahora_local if 'ahora_local' in locals() else datetime.now(ZONA_LOCAL))
    ...
    with t2:
        ...
        if res_i.data:
            ...
            cols = ['id', 'foto_path', 'codigo', 'nombre', 'stock', 'precio_pub', 'precio_inv', 'fecha_ingreso', 'descripcion'] if role == "admin" else ['foto_path', 'codigo', 'nombre', 'stock', 'precio_pub', 'fecha_ingreso', 'descripcion']
            
            df_editado = st.data_editor(
                df_i[[c for c in cols if c in df_i.columns]],
                column_config={"id": None, "foto_path": st.column_config.ImageColumn("Imagen"), "fecha_ingreso": st.column_config.TextColumn("Fecha Ingreso")},
